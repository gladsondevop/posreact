...
import { editForm, saveMiniatura } from "../store/slices/miniatura/action";
...
function Form({isEdit}) {
    ...
    const setFormValues = (field, value) => dispatch(editForm(field, value));
    const {miniatura} = useSelector((state) => state.miniatura);

    const submit = () => dispatch(saveMiniatura(isEdit)).then(() => navigate("/"));
